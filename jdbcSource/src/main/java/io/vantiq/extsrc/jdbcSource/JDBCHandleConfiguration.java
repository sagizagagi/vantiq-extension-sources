...
public class JDBCHandleConfiguration extends Handler<ExtensionServiceMessage> {
    ...
    boolean createDBConnection(Map<String, ?> generalConfig, Map<String, ?> vantiq) {
        // Ensuring that packageRows is set to be true, and failing the configuration
        // otherwise
        if (!(vantiq.get(PACKAGE_ROWS) instanceof String)
                || !(vantiq.get(PACKAGE_ROWS).toString().equalsIgnoreCase("true"))) {
            log.error("Configuration failed. The packageRows field must be set to true.");
            return false;
        }

        int restartAfterNoRequestFromServer = 0;
        if (vantiq.get("restartAfterNoRequestFromServer") instanceof Integer) {
            restartAfterNoRequestFromServer = (Integer) vantiq.get("restartAfterNoRequestFromServer");
        }
                ...
                public void handleMessage(ExtensionServiceMessage message) {
                    try {
                        ...
                        source.client.sendQueryError(replyAddress,
                                "io.vantiq.extsrc.JDBCHandleConfiguration.queryHandler.queuedTasksFull",
                                "The queue of tasks has filled, and as a result the request was unable to be processed.",
                                null);
                    }
                }
    ...
    /**
     * Closes the source {@link JDBCCore} and marks the configuration as completed.
     * The source will
     * be reactivated when the source reconnects, due either to a Reconnect message
     * (likely created by an update to the
     ...
     * sourceConnectionFuture is completed
     * before the configuration can complete, so a program may need to wait before
     * using configured resources.
     * 
